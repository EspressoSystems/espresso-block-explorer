...
import { latestStakeTableCodec } from '../latest_stake_table';
...
describe('LatestStakeTable', () => {
  it('should decode from json', () => {
    ...
    const response = latestStakeTableCodec.decode(JSON.parse(rawString));
    ...
    expect(response.toJSON()).toStrictEqual(
      latestStakeTableCodec.encode(response),
    );

    expect(
      nodeValidatorResponseCodec.decode(JSON.parse(rawString)),
    ).toStrictEqual(latestStakeTableCodec.decode(JSON.parse(rawString)));
