...
describe('LatestStakeTable', () => {
  it('should decode from json', () => {
    ...
    expect(response.toJSON()).toStrictEqual(
      cappuccinoLatestStakeTableCodec.encode(response),
    );

    expect(
      cappuccinoNodeValidatorResponseCodec.decode(JSON.parse(rawString)),
    ).toStrictEqual(
      cappuccinoLatestStakeTableCodec.decode(JSON.parse(rawString)),
    );
