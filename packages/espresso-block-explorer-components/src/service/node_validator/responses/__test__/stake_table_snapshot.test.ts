...
import { stakeTableSnapshotCodec } from '../stake_table_snapshot';
...
describe('StakeTableSnapshot', () => {
  it('should decode from json', () => {
    ...
    const response = stakeTableSnapshotCodec.decode(JSON.parse(rawString));
    ...
    expect(response.toJSON()).toStrictEqual(
      stakeTableSnapshotCodec.encode(response),
    );

    expect(
      nodeValidatorResponseCodec.decode(JSON.parse(rawString)),
    ).toStrictEqual(stakeTableSnapshotCodec.decode(JSON.parse(rawString)));
