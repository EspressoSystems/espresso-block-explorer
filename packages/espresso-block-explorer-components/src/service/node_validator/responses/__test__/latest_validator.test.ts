...
import { latestValidatorCodec } from '../latest_validator';
...
describe('ValidatorsSnapshot', () => {
  it('should decode from json', () => {
    ...
    const response = latestValidatorCodec.decode(JSON.parse(rawString));
    ...
    expect(response.toJSON()).toStrictEqual(
      latestValidatorCodec.encode(response),
    );

    expect(
      nodeValidatorResponseCodec.decode(JSON.parse(rawString)),
    ).toStrictEqual(latestValidatorCodec.decode(JSON.parse(rawString)));
