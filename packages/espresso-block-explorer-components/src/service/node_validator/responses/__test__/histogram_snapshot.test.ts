...
import { histogramSnapshotCodec } from '../histogram_snapshot';
...
describe('HistogramSnapshot', () => {
  it('should decode from json', () => {
    ...
    const response = histogramSnapshotCodec.decode(JSON.parse(rawString));
    ...
    expect(response.toJSON()).toStrictEqual(
      histogramSnapshotCodec.encode(response),
    );

    expect(
      nodeValidatorResponseCodec.decode(JSON.parse(rawString)),
    ).toStrictEqual(histogramSnapshotCodec.decode(JSON.parse(rawString)));
