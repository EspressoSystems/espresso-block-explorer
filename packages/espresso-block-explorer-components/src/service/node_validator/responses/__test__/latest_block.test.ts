...
import { latestBlockCodec } from '../latest_block';
...
describe('LatestBlock', () => {
  it('should decode from json', () => {
    ...
    const response = latestBlockCodec.decode(JSON.parse(rawString));
    ...
    expect(response.toJSON()).toStrictEqual(latestBlockCodec.encode(response));

    expect(
      nodeValidatorResponseCodec.decode(JSON.parse(rawString)),
    ).toStrictEqual(latestBlockCodec.decode(JSON.parse(rawString)));

    expect(nodeValidatorResponseCodec.encode(response)).toStrictEqual(
      latestBlockCodec.encode(response),
    );
  });
});
