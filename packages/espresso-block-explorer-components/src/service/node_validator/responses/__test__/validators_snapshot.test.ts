...
import { validatorsSnapshotCodec } from '../validators_snapshot';
...
describe('ValidatorsSnapshot', () => {
  it('should decode from json', () => {
    ...
    const response = validatorsSnapshotCodec.decode(JSON.parse(rawString));
    ...
    expect(response.toJSON()).toStrictEqual(
      validatorsSnapshotCodec.encode(response),
    );

    expect(
      nodeValidatorResponseCodec.decode(JSON.parse(rawString)),
    ).toStrictEqual(validatorsSnapshotCodec.decode(JSON.parse(rawString)));
