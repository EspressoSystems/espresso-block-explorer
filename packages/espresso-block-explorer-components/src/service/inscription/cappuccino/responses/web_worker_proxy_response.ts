...
import { webWorkerLifeCycleResponseCodec } from './web_worker_life_cycle_response_codec';

export abstract class WebWorkerProxyResponse {}

export const kInscriptionResponseType = 'InscriptionResponse' as const;

export class InscriptionResponse extends WebWorkerProxyResponse {
  ...
  toJson() {
    return inscriptionResponseCodec.encode(this);
  }
}

class InscriptionResponseEncoder implements Converter<InscriptionResponse> {
  convert(input: InscriptionResponse) {
    return {
...
class InscriptionResponseDecoder
  implements Converter<unknown, InscriptionResponse>
{
  convert(input: unknown): InscriptionResponse {
...
class InscriptionResponseCodec extends TypeCheckingCodec<
  InscriptionResponse,
  ReturnType<InstanceType<new () => InscriptionResponseEncoder>['convert']>
> {
  readonly encoder = new InscriptionResponseEncoder();
  readonly decoder = new InscriptionResponseDecoder();
}

export const inscriptionResponseCodec = new InscriptionResponseCodec();
  ...
  implements Converter<CappuccinoInscriptionResponse, WebWorkerProxyResponse>
{
  convert(input: CappuccinoInscriptionResponse): WebWorkerProxyResponse {
    return new InscriptionResponse(input);
  }
}
