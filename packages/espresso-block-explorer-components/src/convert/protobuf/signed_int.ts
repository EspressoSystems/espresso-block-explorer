 ...
 */
export function encodeSint32(number: bigint): bigint {
  const raw = BigInt.asIntN(32, number);
  return BigInt.asUintN(32, (raw << 1n) ^ (raw >> 31n));
}
 ...
 */
export function encodeSint64(number: bigint): bigint {
  const raw = BigInt.asIntN(64, number);
  return (raw << 1n) ^ (raw >> 63n);
}
 ...
 */
export function decodeSint32(number: bigint): bigint {
  if (number & 0x01n) {
    // This is a signed value
    return BigInt.asIntN(32, -((number + 1n) >> 1n));
  }

  return BigInt.asIntN(32, number >> 1n);
}
 ...
 */
export function decodeSint64(number: bigint): bigint {
  if (number & 0x01n) {
    // This is a signed value
    return BigInt.asIntN(64, -((number + 1n) >> 1n));
  }

  return BigInt.asIntN(64, number >> 1n);
}
