...
export interface HotShotQueryService<Leaf, Block, Transaction> {
    readonly availability: HotShotQueryServiceAvailabilityAPI<Leaf, Block, Transaction>;
    readonly status: HotShotQueryServiceStatusAPI;
}
export interface ExtendedHotShotQueryServiceAvailabilityAPI<BlockSummary, TransactionSummary> {
    getBlockSummaries(from: number, until: number): Promise<BlockSummary[]>;
    getTransactionSummaryRange(height: number, offset: number, limit: number): Promise<TransactionSummary[]>;
    getTransactionSummaryRangeForRollup(namespace: number, height: number, offset: number, limit: number): Promise<TransactionSummary[]>;
}
export interface ExtendedHotShotQueryService<BlockSummary, TransactionSummary> {
    readonly availability: ExtendedHotShotQueryServiceAvailabilityAPI<BlockSummary, TransactionSummary>;
}
